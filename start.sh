...
docker run --runtime=nvidia -itd --network samaritan_kafkanet --name $face tlindener/samaritan-face-detection python -u consumer.py --topic $base
docker run --runtime=nvidia -itd --network samaritan_kafkanet --name  $person tlindener/samaritan-person-detection  python -u consumer.py --topic $base
docker run --runtime=nvidia -itd --network samaritan_kafkanet --name  $raw tlindener/samaritan-frame-extractor  python -u producer.py --topic $raw
